...
const CartDelete = ({ id, onClose }) => {
    const { cartid, item } = id;
    const [loading, setLoading] = useState(false);

    const deleteItem = async (cartid, item) => {
        try {
            setLoading(true);
            console.log(cartid + " " + item);
            const response = await axios.post('http://localhost:5000/remove', { cartid, item });
        ...
        <div>
            ...
            <div className="cart-popup">
                <div className="cartpopup-content">
                    <div className="sh">
                    <svg fill="#000000" width="60px" height="45px" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="cart-popup1">
                        <h3>Getting Confirmation From You!</h3>
                    </div>
                    <div className="cart-popup2">
                        <button type="button" id="dele" onClick={() => deleteItem(cartid, item)}>DELETE</button>
                        <button type="button" id="dec" onClick={onClose}>DECLINE</button>
                    </div>
                </div>
            </div>
        </div>
